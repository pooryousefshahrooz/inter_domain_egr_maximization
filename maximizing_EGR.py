...
#     time.sleep(9)
    ...
    for edge in network.set_E:
        if network.end_level_purification_flag:
            opt_model.add_constraint(
                opt_model.sum(x_vars[k,p]*network.each_k_u_weight[k][u] 
    ...
    objective = opt_model.sum(x_vars[k,p]*network.each_k_weight[k] * network.each_k_u_weight[k][u]*network.q_value**(network.get_path_length(p)-1)
                          for k in network.K
                          for u in network.each_k_user_pairs[k] 
                          for p in network.each_k_u_paths[k][u]
                          )
...
def maximizing_EGR():
    ...
    for network_topology,file_path in each_network_topology_file.items():
        for i in range(experiment_repeat):
            for edge_capacity_bound in edge_capacity_bounds:
                network = Network(config,file_path,edge_capacity_bound)
...
fidelity_threshold_ranges = [0.94]
q_values = [1,0.95,0.9,0.8,0.7,0.6]

edge_capacity_bounds = [300]
link_cost_metrics = ["EGR","hop","EGRsquare"]
purification_schemes = ["edge_level","end_level"]
results_file_path = "results/egr5.csv"
